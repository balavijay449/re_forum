$(document).ready(function(){
	...
	$("#submit").click(function(){
		$.ajax({
			url: "faculty_conformation.php",
			type: "post",
			data: $("#form").serialize(),
			success: function(data){
				// student_conformation.php document result is else when this part is run.....
				if(data == "faild"){
					$("#reg_no_text").html("Please Enter correct Id Number").css({"color" : "#FC534C", "font-weight" : "normal"});
					$("#reg_no").css("border-color", "#FC534C");
					...
					$("#reg_no_text").html("Correct Id Number but Password mismatch").css({"color" : "#0AA06E", "font-weight" : "bold"});
					$("#reg_no").css("border-color", "#0AA06E");
